...
function DashboardTab({uid}){
  ...
  const DTABS=[
    ...
    {id:"suivirecrue",label:"📋 Suivi Recrues"},
  ...
  return(
    <div>
      ...
      {dtab==="suivirecrue"&&<SuiviRecruTab uid={uid}/>}
...
const QUESTIONS = {
  ...
  recrutement: [
    {id:"motivation", question:"Quand tu penses à te lancer dans une nouvelle activité, qu'est-ce qui te motive le plus ?", options:[
    ...
    {id:"rejet", question:"Comment réagis-tu quand quelqu'un te dit non ou n'est pas intéressé par ce que tu proposes ?", options:[
    ...
    {id:"regularite", question:"À quelle fréquence pourrais-tu consacrer du temps à une activité, même 20-30 minutes ?", options:[
    ...
    {id:"reseaux", question:"Comment te sens-tu à l'idée de parler de toi, de ton quotidien, sur les réseaux sociaux ?", options:[
    ...
    {id:"boucheaoreille", question:"Si les réseaux sociaux n'étaient pas une option, penses-tu pouvoir développer une activité uniquement via ton entourage et le bouche-à-oreille ?", options:[
    ...
    {id:"apprentissage", question:"Quand tu apprends quelque chose de nouveau (une méthode, un outil), comment réagis-tu ?", options:[
    ...
    {id:"situation", question:"Comment décrirais-tu ta situation financière actuelle / ton besoin ?", options:[
      {value:"a",label:"J'ai vraiment besoin d'un revenu rapide et conséquent", score:1},
      {value:"b",label:"J'aimerais un revenu complémentaire mais sans urgence", score:3},
      {value:"c",label:"Je veux construire un projet sur le moyen/long terme", score:4},
      {value:"d",label:"Je teste surtout par curiosité, sans vraie attente financière", score:2},
    ]},
    {id:"patience", question:"Comment réagis-tu face aux résultats qui prennent du temps à arriver ?", options:[
    ...
    {id:"entraide", question:"As-tu déjà accompagné, conseillé ou aidé des gens autour de toi (même sans contexte professionnel) ?", options:[
    ...
    {id:"envie", question:"Sur une échelle de motivation, où te situes-tu aujourd'hui pour te lancer dans un vrai projet ?", options:[
  ...
  blocage: [
    {id:"duree", question:"Depuis combien de temps as-tu démarré ton activité ?", options:[
    ...
    {id:"contacts", question:"Combien de personnes contactes-tu en moyenne par semaine (réseaux + entourage) ?", options:[
    ...
    {id:"contenu", question:"Publies-tu du contenu sur les réseaux sociaux (posts, stories) ?", options:[
    ...
    {id:"entourage", question:"À l'inverse, parles-tu de ton activité à ton entourage proche (famille, amis, voisins, collègues) ?", options:[
    ...
    {id:"relance", question:"Quand tu envoies un message à un prospect, fais-tu un suivi (relance) si la personne ne répond pas ?", options:[
    ...
    {id:"camera", question:"Comment te sens-tu face à la caméra (lives, vidéos, stories) ?", options:[
    ...
    {id:"routine_pro", question:"As-tu une routine quotidienne ou hebdomadaire pour ton activité (actions planifiées) ?", options:[
    ...
    {id:"discours", question:"Te sens-tu à l'aise pour expliquer simplement ce que tu fais et tes produits/opportunité ?", options:[
    ...
    {id:"frein", question:"Quel est, selon toi, ton plus grand frein actuel ?", options:[
    ...
    {id:"levier", question:"Si tu devais miser sur UN levier pour développer ton activité dans les 30 prochains jours, lequel choisirais-tu naturellement ?", options:[
      {value:"bouche-a-oreille",label:"Le bouche-à-oreille / mon entourage proche", score:3},
      {value:"reseaux",label:"Les réseaux sociaux (posts, stories, lives)", score:3},
      {value:"mix",label:"Un mix des deux, mais je ne sais pas comment m'organiser", score:2},
      {value:"indecise",label:"Je ne sais pas, je n'ai pas de préférence", score:1},
    ]},
  ],
};

// ── DIAGNOSTICS PROFIL (recrutement / blocage) — scoring local, sans IA ──────
function getRecrutementLevel(score, max){
  const pct = score/max;
  if(pct < 0.4) return {
    level:1, label:"Pas le bon moment pour toi",
    desc:"D'après tes réponses, le marketing relationnel demanderait aujourd'hui beaucoup d'efforts de ta part pour des résultats qui prendraient du temps à arriver — ce qui peut vite décourager. Ce n'est absolument pas une question de valeur personnelle : c'est simplement que certains ingrédients (régularité, confiance face aux refus, visibilité) ne sont pas encore réunis pour toi, et ce n'est pas grave.",
    advice:[
      {h:"Pas de pression", t:"Rien ne t'oblige à te lancer maintenant. Le risque, si tu commences trop tôt sans être prête, c'est de t'épuiser pour rien et de te décourager."},
      {h:"Le bon moment peut venir plus tard", t:"Ta situation de vie évolue, tes envies aussi. Ce diagnostic est une photo d'aujourd'hui, pas une décision pour toujours."},
      {h:"Si tu veux quand même essayer", t:"Tu peux commencer doucement, simplement en testant les produits pour toi-même, sans aucune pression de résultat ni d'activité à côté."},
    ]
  };
  if(pct < 0.65) return {
    level:2, label:"Belle marge de progression",
    desc:"Tu as déjà de belles qualités sur lesquelles t'appuyer ! Avec un accompagnement rapproché au démarrage, surtout sur la régularité et la gestion des refus, tu peux construire une activité qui te correspond. Le bouche-à-oreille sera sans doute un terrain plus naturel pour toi que les réseaux sociaux dans un premier temps.",
    advice:[
      {h:"Démarre en douceur", t:"Un démarrage centré sur ton entourage proche (bouche-à-oreille) sera moins anxiogène pour toi que de te lancer directement sur les réseaux."},
      {h:"Un cadre qui rassure", t:"Un point régulier avec ta parraine les premières semaines t'aidera à garder le rythme — c'est ton principal point de vigilance, mais ça se travaille très bien."},
      {h:"Célèbre chaque pas", t:"Chaque petite action compte : un message envoyé, une présentation faite. Ta confiance se construit progressivement, étape par étape."},
    ]
  };
  if(pct < 0.85) return {
    level:3, label:"Tu as un bon profil",
    desc:"Tu as les bases pour réussir : une motivation solide, l'envie d'apprendre, et la capacité à prendre du recul face aux refus. Tu peux te lancer sereinement avec un accompagnement classique, en testant à la fois les réseaux et le bouche-à-oreille pour voir ce qui te correspond le mieux.",
    advice:[
      {h:"Lance-toi sur les deux fronts", t:"Teste à la fois les réseaux et ton entourage dès le départ — tu découvriras rapidement ce qui te convient naturellement."},
      {h:"Une formation structurée t'attend", t:"Le programme START & CASH est parfaitement adapté pour structurer ton démarrage sur les premières semaines."},
      {h:"De l'autonomie, à ton rythme", t:"Tu n'as pas besoin d'un cadre trop serré : prends de l'autonomie rapidement tout en sachant que ta parraine reste disponible."},
    ]
  };
  return {
    level:4, label:"Profil idéal",
    desc:"Ton profil est très prometteur : motivation forte, aisance avec les autres, capacité à rebondir face aux refus, et appétence pour les réseaux ET le bouche-à-oreille. Tu as un vrai potentiel de leadership et tu pourrais toi-même devenir une recruteuse efficace rapidement.",
    advice:[
      {h:"Accélère dès le départ", t:"Lance-toi vite sur le programme START & CASH ET sur MOCHA pour développer ta présence sur les réseaux dès le début."},
      {h:"Pense déjà équipe", t:"Tu peux te projeter assez vite sur la dimension équipe/leadership — ton profil suggère que tu peux viser le développement d'une équipe rapidement."},
      {h:"Ton énergie attire", t:"N'hésite pas à partager ton parcours dès le début — ton énergie sera un atout pour attirer d'autres personnes qui te ressemblent."},
    ]
  };
}

function getRecrutementInternalNote(reponsesArr, result){
  // index 3 = aisance réseaux, index 4 = capacité bouche-à-oreille
  const reseauxScore = reponsesArr[3]?.score || 0;
  const boucheScore = reponsesArr[4]?.score || 0;
  let levier;
  if(reseauxScore >= 3 && boucheScore >= 3) levier = "Les deux leviers semblent accessibles — propose un démarrage mixte.";
  else if(reseauxScore > boucheScore) levier = "Démarre plutôt par les réseaux sociaux (MOCHA) : c'est là qu'elle semble le plus à l'aise.";
  else if(boucheScore > reseauxScore) levier = "Démarre plutôt par le bouche-à-oreille / entourage : c'est le terrain le plus naturel pour elle.";
  else levier = "Pas de tendance claire — propose un test sur les deux leviers pour observer ce qui fonctionne le mieux.";

  let action;
  if(result.level === 1) action = "Ne pas pousser vers une inscription. Proposer plutôt une expérience produit, sans pression d'activité. Garder le contact léger pour un futur moment plus opportun.";
  else if(result.level === 2) action = "Prévoir un point de suivi rapproché (hebdo) les 2-3 premières semaines. Démarrer en douceur sur le bouche-à-oreille. Valoriser chaque petite action pour construire la confiance.";
  else if(result.level === 3) action = "Lancer le programme START & CASH. Encourager un test sur les deux leviers dès le départ. Autonomie rapide, suivi disponible mais pas intrusif.";
  else action = "Profil prioritaire : démarrer START & CASH + MOCHA en parallèle. Évoquer rapidement la dimension équipe/leadership. Excellent profil à mettre en avant dans des témoignages.";

  return {levier, action};
}

function getBlocageOrientation(score, max, reponsesArr){
  const lastAnswer = reponsesArr[reponsesArr.length-1];
  const preferredLever = lastAnswer ? lastAnswer.value : null;
  ...
  if(preferredLever === "reseaux"){
    orientation = {
      ...
      actions:[
        {h:"Programme MOCHA", t:"Reprends (ou refais) le programme MOCHA pour structurer ta présence sur les réseaux : régularité de publication, formats qui convertissent, CTA par mots-clés."},
    ...
    orientation = {
      ...
      actions:[
        {h:"Liste chaude", t:"Reconstruis (ou réactive) ta liste de contacts proches : famille, amis, anciens collègues, voisins — avec un angle naturel (partage d'expérience produit plutôt que pitch direct)."},
        ...
        {h:"Suivi systématique", t:"Travaille la relance : beaucoup de blocages viennent de l'absence de suivi après un premier contact. Un rappel structuré peut t'aider."},
      ]
    };
  } else if(preferredLever === "mix"){
    orientation = {
      title:"Orientation : Mix réseaux + entourage, avec un cadre",
      desc:"Tu vois l'intérêt des deux leviers mais manques d'organisation pour les mener de front. Il te faut un cadre simple.",
      actions:[
        {h:"Planning simplifié", t:"Répartis simplement : par exemple lundi/mercredi/vendredi = contenu réseaux, mardi/jeudi = contacts entourage. Pas plus de 2-3 actions par jour."},
    ...
    orientation = {
      ...
      actions:[
        {h:"Discussion exploratoire", t:"Prends un temps avec ta parraine pour explorer où tu te sens le plus à l'aise naturellement : parler à des gens que tu connais, ou créer du contenu en ligne."},
        ...
        {h:"Réassurance", t:"L'indécision cache souvent une peur de mal faire. Il n'y a pas de mauvais choix, seulement des essais à ajuster."},
      ]
    };
  }
  ...
  if(pct < 0.4) levelInfo = {level:1, label:"Blocage profond — réassurance prioritaire",
    ...
    extra:"Tu as des bases mais manques de structure. Un planning simple et des objectifs atteignables sur 2-3 semaines devraient permettre de relancer la dynamique."};
    ...
    extra:"Le score est élevé : il ne s'agit probablement pas d'un blocage profond mais d'un ajustement de stratégie ou d'un passage à un niveau supérieur (par exemple : viser le leadership d'équipe)."};

  return {orientation, levelInfo};
}

// Construit le résultat "profil" pour recrutement/blocage à partir des réponses brutes {questionId: value}
function genererResultatProfil(type, reponses){
  const questions = QUESTIONS[type];
  const reponsesArr = questions.map(q=>{
    const val = reponses[q.id];
    const opt = q.options.find(o=>o.value===val);
    return { value: val, score: opt ? (opt.score||0) : 0 };
  });
  const score = reponsesArr.reduce((s,a)=>s+a.score,0);
  const max = questions.length * 4;

  if(type === "recrutement"){
    const result = getRecrutementLevel(score, max);
    const internalNote = getRecrutementInternalNote(reponsesArr, result);
    return { kind:"profil", profilType:"recrutement", score, max, level:result.level, levelLabel:result.label, desc:result.desc, advice:result.advice, internalNote };
  } else {
    const {orientation, levelInfo} = getBlocageOrientation(score, max, reponsesArr);
    return { kind:"profil", profilType:"blocage", score, max, level:levelInfo.level, levelLabel:levelInfo.label, desc:levelInfo.extra, orientation };
  }
}
...
function DiagnosticsTab({ uid, userName }) {
  ...
  const genererOrdonnance = async (rep) => {
    if(type === "recrutement" || type === "blocage"){
      const result = genererResultatProfil(type, rep);
  ...
  const saveResult = async (reco, rep) => {
    const newDiag = { id: `diag${Date.now()}`, type, nomClient: nomClient||(type==="recrutement"||type==="blocage"?"Sans nom":"Cliente"), reponses: rep||reponses, ordonnance: reco, date: new Date().toISOString().slice(0,10), ts: Date.now() };
    try {
      const ref = doc(db, "users", uid);
      const snap = await getDoc(ref);
      ...
      // Pour les diagnostics recrutement/blocage : le chef d'équipe reçoit aussi une copie
      if(type === "recrutement" || type === "blocage"){
        const chefId = snap.exists() ? snap.data()["chef-equipe"] : null;
        if(chefId){
          const chefRef = doc(db, "users", chefId);
          const chefSnap = await getDoc(chefRef);
          const chefExisting = chefSnap.exists() && chefSnap.data()["db-diagnostics-equipe"] ? JSON.parse(chefSnap.data()["db-diagnostics-equipe"]) : [];
          const newDiagPourChef = { ...newDiag, recrueNom: userName, recrueUid: uid };
          await setDoc(chefRef, { "db-diagnostics-equipe": JSON.stringify([newDiagPourChef,...chefExisting].slice(0,100)) }, { merge: true });
        }
      }
    } catch {}
  };
  ...
  const TYPES_DIAG = [
    ...
    { id: "recrutement", icon: "🧭", label: "Diagnostic Recrutement", desc: "Profil face au marketing relationnel — pour une prospecte" },
    { id: "blocage", icon: "🔄", label: "Diagnostic Recrue bloquée", desc: "Identifie le blocage et oriente vers un plan d'action" },
  ];
  ...
  if (mode === "choix") return (
    <div>
      ...
      <div style={{ marginBottom: ".75rem" }}>
        <div style={{ fontSize: ".62rem", color: C.gris, marginBottom: ".4rem" }}>Nom de la cliente (optionnel)</div>
  ...
  if (mode === "resultat" && ordonnance && ordonnance.kind === "profil") {
    const levelColors = {1:"#B04040",2:C.or,3:C.vert,4:C.rose};
    const lc = levelColors[ordonnance.level] || C.rose;
    return (
      <div>
        ...
        <div style={{ display:"inline-block", background: lc+"20", color: lc, fontSize:".62rem", fontWeight:700, textTransform:"uppercase", letterSpacing:".08em", padding:".25rem .7rem", borderRadius:20, marginBottom:".6rem" }}>
          Niveau {ordonnance.level} / 4 — {ordonnance.levelLabel}
        </div>

        <div style={{ fontFamily: "Georgia,serif", fontSize: "1.3rem", color: C.brun, marginBottom: ".2rem" }}>
          {ordonnance.profilType === "recrutement" ? (nomClient ? `Ton résultat, ${nomClient}` : "Ton résultat") : (nomClient || "Résultat")}
        </div>
        <div style={{ fontFamily: "Georgia,serif", fontStyle:"italic", fontSize: "2.2rem", color: lc, marginBottom:".5rem" }}>
          {ordonnance.score} / {ordonnance.max}
        </div>
        <p style={{ fontSize: ".8rem", color: C.texte, lineHeight: 1.65, marginBottom: "1rem" }}>{ordonnance.desc}</p>

        {ordonnance.profilType === "recrutement" && ordonnance.advice.map((a,i)=>(
          <div key={i} style={{ background: C.creme, borderLeft:`4px solid ${C.lilas}`, borderRadius:"0 10px 10px 0", padding:".7rem .9rem", marginBottom:".55rem" }}>
            <div style={{ fontFamily:"Georgia,serif", fontSize:".88rem", color:C.brun, marginBottom:".2rem" }}>{a.h}</div>
            <p style={{ fontSize:".74rem", color:C.gris, lineHeight:1.6, margin:0 }}>{a.t}</p>
          </div>
        ))}

        {ordonnance.profilType === "blocage" && (
          <>
            <div style={{ background: C.rose+"15", borderLeft:`4px solid ${C.rose}`, borderRadius:"0 10px 10px 0", padding:".7rem .9rem", marginBottom:".55rem" }}>
              <div style={{ fontFamily:"Georgia,serif", fontSize:".88rem", color:C.brun, marginBottom:".2rem" }}>{ordonnance.orientation.title}</div>
              <p style={{ fontSize:".74rem", color:C.gris, lineHeight:1.6, margin:0 }}>{ordonnance.orientation.desc}</p>
            </div>
            {ordonnance.orientation.actions.map((a,i)=>(
              <div key={i} style={{ background: C.creme, borderLeft:`4px solid ${C.lilas}`, borderRadius:"0 10px 10px 0", padding:".7rem .9rem", marginBottom:".55rem" }}>
                <div style={{ fontFamily:"Georgia,serif", fontSize:".88rem", color:C.brun, marginBottom:".2rem" }}>{a.h}</div>
                <p style={{ fontSize:".74rem", color:C.gris, lineHeight:1.6, margin:0 }}>{a.t}</p>
              </div>
        ...
        {ordonnance.profilType === "recrutement" && ordonnance.internalNote && (
          <div style={{ background: C.lilas+"15", borderLeft:`4px solid ${C.lilas}`, borderRadius:"0 10px 10px 0", padding:".7rem .9rem", marginTop:".75rem" }}>
            <div style={{ fontFamily:"Georgia,serif", fontSize:".88rem", color:C.brun, marginBottom:".3rem" }}>👀 Pour toi (note interne)</div>
            <p style={{ fontSize:".74rem", color:C.gris, lineHeight:1.6, margin:0 }}><strong>Levier conseillé :</strong> {ordonnance.internalNote.levier}</p>
            <p style={{ fontSize:".74rem", color:C.gris, lineHeight:1.6, margin:".3rem 0 0" }}><strong>Action recommandée :</strong> {ordonnance.internalNote.action}</p>
            <p style={{ fontSize:".62rem", color:C.pale, marginTop:".4rem" }}>Cette note est privée et reste visible uniquement dans "Mes diagnostics" (et chez ta chef d'équipe).</p>
          </div>
        )}

        <p style={{ fontSize: ".65rem", color: C.gris, textAlign: "center", marginTop:"1rem" }}>Résultat sauvegardé dans ton tableau de bord 🖤{type==="recrutement"||type==="blocage" ? " — une copie est envoyée à ta chef d'équipe." : ""}</p>
      </div>
    );
  }
...
function DiagResultsTab({ uid }) {
  ...
  const TYPE_LABELS = { skincare: "✨ Skincare", cheveux: "💇 Cheveux", sante: "💊 Santé" };
