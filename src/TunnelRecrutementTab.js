...
export function TunnelRecrutementTab({ uid, userName, db }) {
  ...
  const [showChoixLien, setShowChoixLien] = useState(false);

  const slug = (userName || uid).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]/g, '-');
  const lienPublic = `${window.location.origin}/?tunnel=${slug}`;
  ...
  const sauver = async () => {
    ...
    try {
      ...
      // Egalement dans contacts_publics (lecture publique, accessible aux visiteuses anonymes)
      await setDoc(doc(db, 'contacts_publics', uid), { tunnelRecrutementConfig: cfg }, { merge: true });
  ...
  const copyLinkSeul = async () => {
    ...
    navigator.clipboard.writeText(lienCourt);
    ...
    <div style={{ fontFamily: 'inherit', maxWidth: 480, margin: '0 auto', padding: '0 0 3rem' }}>
      ...
      <div style={{ background: `linear-gradient(135deg, ${CR.brun}, #3D2020)`, borderRadius: 16, padding: '1.2rem', marginBottom: '1rem', color: 'white' }}>
        ...
        <div style={{ display: 'flex', gap: '.5rem', position:'relative' }}>
          <button onClick={() => setShowChoixLien(!showChoixLien)} style={{ flex: 1, background: CR.or, color: 'white', border: 'none', borderRadius: 8, padding: '.5rem', fontSize: '.75rem', fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer' }}>📋 Copier le lien</button>
          {showChoixLien&&(
            <div style={{position:'absolute',top:'110%',left:0,zIndex:20,background:'white',border:'1px solid #E8DDD4',borderRadius:10,boxShadow:'0 4px 16px rgba(0,0,0,.2)',overflow:'hidden',minWidth:180}}>
              <button onClick={()=>{copyLinkSeul();setShowChoixLien(false);}}
                style={{width:'100%',background:'white',border:'none',borderBottom:'1px solid #E8DDD4',padding:'.6rem .75rem',fontSize:'.72rem',fontWeight:600,color:'#3D2B1F',cursor:'pointer',fontFamily:'inherit',textAlign:'left'}}>
                🔗 Lien seul
              </button>
              <button onClick={()=>{copyLink();setShowChoixLien(false);}}
                style={{width:'100%',background:'white',border:'none',padding:'.6rem .75rem',fontSize:'.72rem',fontWeight:600,color:'#3D2B1F',cursor:'pointer',fontFamily:'inherit',textAlign:'left'}}>
                💬 Lien avec message
              </button>
            </div>
          )}
...
export function TunnelRecrutementPublic({ slug, db, preview, previewCfg, previewTokens }) {
  ...
  const [diagTunnel, setDiagTunnel] = useState(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        diag.contactsPubliquesExiste = snapPub.exists();
        let uid = slug;
        let cfgTrouve = null;
        if (snapPub.exists()) {
          uid = snapPub.data().uid || slug;
          diag.uidDansDoc = snapPub.data().uid || "(absent, utilise slug)";
          diag.aTunnelRecrutementConfig = !!snapPub.data().tunnelRecrutementConfig;
          if (snapPub.data().tunnelRecrutementConfig) cfgTrouve = snapPub.data().tunnelRecrutementConfig;
        }
        // Repli : ancienne collection (peut echouer pour une visiteuse anonyme selon les regles de securite)
        if(!cfgTrouve){
          try{
            let snapCfg = await getDoc(doc(db, 'tunnel_recrutement', uid));
            diag.ancienneCollectionExiste = snapCfg.exists();
            if(!snapCfg.exists() && uid!==slug){
              const snapCfg2 = await getDoc(doc(db, 'tunnel_recrutement', slug));
              if(snapCfg2.exists()){ uid = slug; snapCfg = snapCfg2; }
            }
            if(snapCfg.exists()) cfgTrouve = snapCfg.data();
          }catch(e){ diag.erreurAncienneCollection = e.message; }
        }
        diag.cfgFinalTrouve = !!cfgTrouve;
        diag.cfgActif = cfgTrouve ? !!cfgTrouve.actif : null;
        if (cfgTrouve) setCfg(cfgTrouve);
      ...
      } catch(e) { diag.erreurGenerale = e.message; }
      setDiagTunnel(diag);
  ...
  if (!cfg || !cfg.actif) return (
    <div style={{ textAlign: 'center', padding: '3rem 1.5rem', color: CR.gris }}>
      <div>Page non disponible.</div>
      {diagTunnel&&(
        <div style={{marginTop:"1.2rem",fontSize:".68rem",color:"#888",background:"#FFF3F0",border:"1px solid #F4C0B0",borderRadius:8,padding:".7rem .9rem",textAlign:"left",display:"inline-block",lineHeight:1.7}}>
          🔍 slug: {diagTunnel.slug}<br/>
          contacts_publics existe: {String(diagTunnel.contactsPubliquesExiste)}<br/>
          uid dans le doc: {diagTunnel.uidDansDoc||"(n/a)"}<br/>
          a tunnelRecrutementConfig: {String(diagTunnel.aTunnelRecrutementConfig)}<br/>
          ancienne collection existe: {String(diagTunnel.ancienneCollectionExiste)}<br/>
          config finale trouvée: {String(diagTunnel.cfgFinalTrouve)}<br/>
          config active: {String(diagTunnel.cfgActif)}<br/>
          {diagTunnel.erreurAncienneCollection&&<>erreur ancienne collection: {diagTunnel.erreurAncienneCollection}<br/></>}
          {diagTunnel.erreurGenerale&&<>erreur générale: {diagTunnel.erreurGenerale}<br/></>}
        </div>
      )}
    </div>
  );
    ...
    <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif", background: CR.creme, minHeight: '100vh', maxWidth: 520, margin: '0 auto' }}>
      ...
      <div style={{ margin: '0 1rem 1rem', background: 'white', border: `1px solid ${CR.pale}`, borderRadius: 14, padding: '1.2rem' }}>
        ...
        <div style={{ margin: '0 1rem 1rem', background: 'linear-gradient(135deg,' + CR.brun + ',#3D2020)', borderRadius: 14, padding: '1.4rem', color: 'white', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '.72rem', opacity: .7, fontStyle: 'italic' }}>{prenom} accompagne personnellement chaque nouvelle recrue</div>
        </div>
