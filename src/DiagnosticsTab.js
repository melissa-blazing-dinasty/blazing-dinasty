...
async function genererOrdonnanceIA(type, reponses, nomClient) {
  ...
  try {
    ...
    if(catSnap.exists()){
      ...
      const typeKey = type==="skincare"?"skincare"
        :type==="cheveux"?"cheveux"
        :(type==="sante"||type==="silhouette"||type==="detox"||type==="antiage")?"sante"
        :type==="makeup"?"makeup"
        :type==="peaucorps"?"peaucorps"
        :type==="parfum"?"parfum"
        :null;
      ...
      if(typeKey==="skincare"||typeKey==="cheveux"||typeKey==="peaucorps"){
        ...
        complementsText = comp.slice(0,15).map((p,i)=>(i+1)+". "+p.nom+" - "+(p.prix!=null?p.prix:"?")+"EUR"+(p.prixVIP?" (prix VIP : "+p.prixVIP+"EUR)":"")).join("\n");
      }
    }
  ...
  const isSkincareOuCheveux = (type==="skincare"||type==="cheveux"||type==="peaucorps");
      ...
      <div id="decouverte-diag-liste">{TYPES_DIAG.filter(t=>t.cat===catDiag).map(t => (
        <div key={t.id} style={{ background: C.blanc, border: `1px solid ${C.pale}`, borderRadius: 14, padding: "1rem 1.1rem", marginBottom: ".65rem" }}>
          ...
          <div style={{ display: "flex", gap: ".4rem", position:"relative" }}>
            ...
            <button onClick={()=>setChoixLienPour(choixLienPour===t.id?null:t.id)}
            ...
            {choixLienPour===t.id&&(
              <div style={{position:"absolute",top:"110%",right:0,zIndex:20,background:"white",border:`1px solid ${C.pale}`,borderRadius:10,boxShadow:"0 4px 16px rgba(0,0,0,.15)",overflow:"hidden",minWidth:180}}>
                <button onClick={()=>{copierLienNu(t.id);setChoixLienPour(null);}}
                  style={{width:"100%",background:"white",border:"none",borderBottom:`1px solid ${C.pale}`,padding:".6rem .75rem",fontSize:".72rem",fontWeight:600,color:C.texte,cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}>
                  🔗 Lien seul
                </button>
                <button onClick={()=>{copierLienDirect(t.id,"");setChoixLienPour(null);}}
                  style={{width:"100%",background:"white",border:"none",padding:".6rem .75rem",fontSize:".72rem",fontWeight:600,color:C.texte,cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}>
                  💬 Lien avec message
                </button>
              </div>
            )}
          ...
          <div style={{display:"flex",gap:".5rem",padding:".85rem 1rem",background:theme.bg}}>
            ...
            {profil.lienRecrutement&&(
              <a href={profil.lienRecrutement} target="_blank" rel="noopener noreferrer" onClick={()=>trackerEvenement(profil.slug||slug,"clic_recrutement_externe")}
                style={{flex:1,background:"linear-gradient(135deg,#E8A33D,#C44B1A)",color:"white",textDecoration:"none",textAlign:"center",padding:"1rem .6rem",borderRadius:14,fontSize:".88rem",fontWeight:800,boxShadow:"0 6px 20px rgba(196,75,26,.5)",border:"2px solid #FFD98A",letterSpacing:".01em"}}>
...
function BoutiquePubliquePage({slug}){
  ...
  const THEMES_BOUTIQUE={
    ...
    rose_gold:{header:"linear-gradient(135deg,#C49A8A,#A0716A)",accent:"#C49A8A",btn:"#5C3020",bgPage:"#FFF5F3"},
    nuit:{header:"linear-gradient(135deg,#0D0D2B,#1A1A4E)",accent:"#A89BB5",btn:"#0D0D2B",bgPage:"#F1F0F7"},
    or_noir:{header:"linear-gradient(135deg,#1A1A1A,#2D2D2D)",accent:"#C4A832",btn:"#1A1A1A",bgPage:"#FAFAF5"},
    nature:{header:"linear-gradient(135deg,#2E7D32,#388E3C)",accent:"#2E7D32",btn:"#1B5E20",bgPage:"#F1F8F1"},
    lavande:{header:"linear-gradient(135deg,#7E57C2,#9575CD)",accent:"#7E57C2",btn:"#4527A0",bgPage:"#F5F2FA"},
    soleil:{header:"linear-gradient(135deg,#F57F17,#F9A825)",accent:"#F57F17",btn:"#E65100",bgPage:"#FFFBF0"},
    ocean:{header:"linear-gradient(135deg,#1565C0,#1976D2)",accent:"#1565C0",btn:"#0D47A1",bgPage:"#F0F8FF"},
    corail:{header:"linear-gradient(135deg,#FF6B6B,#FF8E8E)",accent:"#FF6B6B",btn:"#C62828",bgPage:"#FFF9F7"},
    minimaliste:{header:"linear-gradient(135deg,#212121,#424242)",accent:"#212121",btn:"#000",bgPage:"#FAFAFA"},
  };
  ...
  const ajouterPanier=(prod)=>{
    ...
    setCart(c=>{
      ...
      if(profil&&profil.livraisonGratuite){
        ...
        if(nouveauTotal>=60) setToast({type:"success",text:"🎉 Livraison offerte débloquée !"});
        else setToast({type:"info",text:"Encore "+(60-nouveauTotal).toFixed(2)+"€ pour la livraison offerte"});
      }
      return nouveauCart;
    });
  };
  const changerQuantite=(ref,delta)=>{
    setCart(c=>c.map(i=>i.ref===ref?{...i,quantite:i.quantite+delta}:i).filter(i=>i.quantite>0));
  };
  const retirerArticle=(ref)=>{setCart(c=>c.filter(i=>i.ref!==ref));};
  ...
  const livraisonGratuiteActivee=profil&&profil!=="404"&&!!profil.livraisonGratuite;
  const portOffert=livraisonGratuiteActivee&&totalPanier>=SEUIL_GRATUIT;
  ...
  if(loading)return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#FAF7F2"}}>
      <div style={{textAlign:"center"}}>
        <div style={{fontFamily:"Georgia,serif",fontSize:"1.5rem",color:"#3D1F0E"}}>Blazing <em style={{color:"#C49A8A"}}>Dynasty</em></div>
        <div style={{fontSize:".75rem",color:"#888",marginTop:".5rem"}}>Chargement de la boutique...</div>
      </div>
    </div>
  );
  ...
  if(!profil||profil==="404")return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#FAF7F2"}}>
      <div style={{textAlign:"center",padding:"2rem"}}>
        <div style={{fontSize:"2.5rem",marginBottom:"1rem"}}>🔍</div>
        <div style={{fontFamily:"Georgia,serif",fontSize:"1.2rem",color:"#3D1F0E",marginBottom:".5rem"}}>Boutique introuvable</div>
        <div style={{fontSize:".78rem",color:"#888"}}>Ce lien ne correspond à aucune distributrice active.</div>
      </div>
    </div>
  );
  ...
  return(
    <div style={{minHeight:"100vh",background:theme.bgPage,fontFamily:"'Trebuchet MS',sans-serif",paddingBottom:nbArticles?90:0,transition:"background .3s"}}>
      <style>{`
        .boutique-container{max-width:480px;}
        @media (min-width:768px){.boutique-container{max-width:900px;}}
        @media (min-width:1100px){.boutique-container{max-width:1200px;}}
        @keyframes boutique-toast-in{from{opacity:0;transform:translate(-50%,-8px)}to{opacity:1;transform:translate(-50%,0)}}
        .boutique-card{transition:box-shadow .2s ease,transform .15s ease;box-shadow:0 1px 3px rgba(0,0,0,.05);}
        .boutique-card:hover{box-shadow:0 6px 16px rgba(0,0,0,.09);transform:translateY(-2px);}
        .boutique-btn{transition:filter .15s ease,transform .1s ease;}
        .boutique-btn:active{transform:scale(.97);}
      `}</style>
      {toast&&(
        <div style={{position:"fixed",top:16,left:"50%",zIndex:300,background:toast.type==="success"?"#2E7D32":"#3D1F0E",color:"white",padding:".65rem 1.1rem",borderRadius:30,fontSize:".8rem",fontWeight:700,boxShadow:"0 6px 20px rgba(0,0,0,.2)",animation:"boutique-toast-in .25s ease",whiteSpace:"nowrap"}}>
          {toast.text}
        </div>
      )}
      <div className="boutique-container" style={{margin:"0 auto",minHeight:"100vh",background:theme.bgPage,position:"relative"}}>
        ...
        <div style={{background:theme.header,padding:"1.5rem 1rem 1.25rem",textAlign:"center",position:"relative"}}>
          ...
          <div style={{fontSize:".62rem",fontWeight:700,letterSpacing:".15em",color:theme.accent,marginBottom:".2rem"}}>🛍️ BOUTIQUE EN LIGNE</div>
        ...
        {retourCommande==="succes"&&retourFidelite&&showSuccessBanner&&profil.bestSellers&&profil.bestSellers.length>0&&catalogue&&(
          <div style={{margin:"0 1rem 1rem",padding:"1rem",background:theme.bgPage,border:`1.5px solid ${theme.accent}`,borderRadius:14}}>
            <div style={{fontSize:".8rem",fontWeight:700,color:"#3D1F0E",marginBottom:".2rem"}}>🎁 Merci pour ta fidélité !</div>
            <div style={{fontSize:".72rem",color:"#666",marginBottom:".6rem",lineHeight:1.5}}>Voici une petite sélection rien que pour toi :</div>
            <div style={{display:"flex",gap:".5rem",overflowX:"auto"}}>
              {profil.bestSellers.slice(0,6).map(ref=>{
                const prod=Object.values(catalogue).flat().find(p=>p.ref===ref);
                if(!prod)return null;
                return(
                  <div key={ref} style={{flexShrink:0,width:90,textAlign:"center"}}>
                    {prod.image?<img src={prod.image} alt={prod.nom} style={{width:70,height:70,objectFit:"cover",borderRadius:8,margin:"0 auto .3rem"}}/>:<div style={{width:70,height:70,borderRadius:8,background:"white",margin:"0 auto .3rem",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.3rem"}}>🧴</div>}
        ...
        <div style={{display:"flex",gap:".4rem",overflowX:"auto",padding:".85rem 1rem",WebkitOverflowScrolling:"touch"}}>
          {allCategoriesTabs.map(cat=>(
            <button key={cat} className="boutique-btn" onClick={()=>setActiveCat(cat)}
              style={{flexShrink:0,background:activeCat===cat?theme.accent:"white",color:activeCat===cat?"white":"#3D1F0E",border:`1.5px solid ${theme.accent}`,borderRadius:20,padding:".45rem 1rem",fontSize:".74rem",fontWeight:700,fontFamily:"inherit",cursor:"pointer",whiteSpace:"nowrap"}}>
              {CAT_LABELS_FULL[cat]}
            </button>
          ))}
        </div>
            ...
            return(
              <div key={prod.ref} id={"item-"+prod.ref} className="boutique-card" style={{background:"white",borderRadius:12,padding:".55rem .45rem",border:itemSurligne===prod.ref?`2px solid ${theme.accent}`:"1px solid #E8DDD4",boxShadow:itemSurligne===prod.ref?`0 0 0 4px ${theme.accent}30`:"none",display:"flex",flexDirection:"column",position:"relative",transition:"box-shadow .3s, border-color .3s",opacity:prod.rupture?.75:1}}>
                ...
                {prod.rupture?(
                  ...
                  <button className="boutique-btn" onClick={()=>ajouterPanier(prod)}
                    style={{width:"100%",background:theme.btn,color:"white",border:"none",borderRadius:7,padding:".35rem",fontSize:".62rem",fontWeight:700,fontFamily:"inherit",cursor:"pointer"}}>
                    + Ajouter
                  </button>
                )}
      ...
      {nbArticles>0&&!showCart&&(
        <div className="boutique-container" onClick={()=>setShowCart(true)}
          style={{position:"fixed",bottom:0,left:0,right:0,margin:"0 auto",background:theme.btn,padding:".85rem 1rem",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer",boxShadow:"0 -4px 15px rgba(0,0,0,.15)"}}>
          <div style={{color:"white",fontSize:".8rem",fontWeight:700}}>🛒 {nbArticles} article{nbArticles>1?"s":""}</div>
      ...
      {showCart&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:200,display:"flex",alignItems:"flex-end"}}>
          <div style={{background:"white",width:"100%",maxWidth:480,margin:"0 auto",borderRadius:"20px 20px 0 0",maxHeight:"85vh",overflowY:"auto",padding:"1.25rem"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1rem"}}>
              <div style={{fontFamily:"Georgia,serif",fontSize:"1.1rem",color:"#3D1F0E"}}>🛒 Mon panier</div>
              <button onClick={()=>{setShowCart(false);setShowCheckout(false);setCheckoutError("");}} style={{background:"none",border:"none",fontSize:"1.3rem",color:"#888",cursor:"pointer"}}>✕</button>
            </div>

            {!showCheckout?(<>
              {cart.map(item=>(
                <div key={item.ref} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:".6rem 0",borderBottom:"1px solid #F0EBE3"}}>
                  <div style={{flex:1}}>
                    <div style={{fontSize:".78rem",fontWeight:700,color:"#3D1F0E"}}>{item.nom}</div>
                    <div style={{fontSize:".72rem",color:"#888"}}>{item.prix.toFixed(2)}€ x {item.quantite}</div>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:".5rem"}}>
                    <button className="boutique-btn" onClick={()=>changerQuantite(item.ref,-1)} style={{background:theme.bgPage,border:"none",borderRadius:6,width:26,height:26,fontSize:"1rem",fontWeight:700,color:theme.accent,cursor:"pointer"}}>−</button>
                    <span style={{fontSize:".78rem",fontWeight:700,minWidth:16,textAlign:"center"}}>{item.quantite}</span>
                    <button className="boutique-btn" onClick={()=>changerQuantite(item.ref,1)} style={{background:theme.bgPage,border:"none",borderRadius:6,width:26,height:26,fontSize:"1rem",fontWeight:700,color:theme.accent,cursor:"pointer"}}>+</button>
                    <button onClick={()=>retirerArticle(item.ref)} style={{background:"none",border:"none",fontSize:".9rem",color:"#C44B1A",cursor:"pointer",marginLeft:".2rem"}}>🗑️</button>
                  </div>
                </div>
              ))}
              {livraisonGratuiteActivee&&!portOffert&&(
                <div style={{fontSize:".68rem",color:"#C44B1A",padding:".5rem 0",lineHeight:1.5}}>
                  Ajoute {(SEUIL_GRATUIT-totalPanier).toFixed(2)}€ d'achat pour la livraison offerte !
                </div>
              )}
              <div style={{display:"flex",justifyContent:"space-between",padding:".3rem 0",fontSize:".78rem",color:"#666"}}>
                <span>Sous-total</span><span>{totalPanier.toFixed(2)}€</span>
              </div>
              <div style={{display:"flex",justifyContent:"space-between",padding:".3rem 0",fontSize:".78rem",color:"#666"}}>
                <span>Livraison</span><span>{portOffert?"Offerte 🎉":fraisPort.toFixed(2)+"€"}</span>
              </div>
              ...
              <div style={{fontSize:".72rem",color:"#888",marginBottom:".85rem",lineHeight:1.5}}>Pour finaliser ta commande de {totalAvecPort.toFixed(2)}€ (dont {portOffert?"livraison offerte":fraisPort.toFixed(2)+"€ de livraison"}), indique tes coordonnées :</div>
              <input placeholder="Prénom et nom *" value={clientNom} onChange={e=>setClientNom(e.target.value)}
          ...
          <div key={qi} style={{background:"white",borderRadius:14,padding:"1rem",marginBottom:"1rem",border:"1px solid #E8DDD4"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:".4rem"}}>
              {item.opts.map(([ic,label,val])=>(
                <button key={val} onClick={()=>{setParcours("produits");setDiagType(val);setEtape("diag_produit");}} style={{display:"flex",alignItems:"center",gap:".75rem",background:"#FAF7F2",border:"1.5px solid #E8DDD4",borderRadius:10,padding:".65rem .85rem",cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}><span style={{fontSize:"1.2rem"}}>{ic}</span><span style={{fontSize:".82rem",color:"#3D2B1F",fontWeight:600}}>{label}</span></button>
              ))}
            </div>
          </div>
        ))}
...
export { DiagnosticParfumTab, DiagnosticsTab, DiagResultsTab, LinkBioPublicPage, BoutiquePubliquePage, TunnelHybridePage, RecommandationPubliquePage, FORMATION_APP_CATEGORIES, FORMATION_APP_CATEGORIES_DEFAULT };
export default Root;
