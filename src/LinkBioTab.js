...
function LinkBioTab({uid, userName, initialSection="theme"}){
  ...
  const[activeSection,setActiveSection]=useState(initialSection); // theme|profil|liens|banniere|photos
  ...
  const [packEnPromo,setPackEnPromo]=useState(false);
  const [packPrixPromo,setPackPrixPromo]=useState("");
  ...
  useEffect(()=>{
    if(activeSection!=="boutique")return;
    (async()=>{
      try{
        const snap=await getDoc(doc(db,"users",uid));
        if(snap.exists()){
          setLienPaypalMe(snap.data()["db-lien-paypalme"]||"");
          setLienStripePerso(snap.data()["db-lien-stripe-perso"]||"");
        }
      }catch{}
    })();
  },[activeSection]);
  const sauverPaiementPerso=async()=>{
    ...
    try{
      ...
      await setDoc(doc(db,"contacts_publics",uid),{
        "db-lien-paypalme":lienPaypalMe.trim(),
        "db-lien-stripe-perso":lienStripePerso.trim()
      },{merge:true});
        ...
        <div>
          ...
          {profil.boutiqueActive&&(<>
            <div style={{background:"#F4F0FF",borderRadius:12,padding:"1rem",marginBottom:"1rem",border:"1px solid #D8CCFF"}}>
              ...
              <div style={{fontSize:".64rem",color:"#888",marginBottom:".7rem",lineHeight:1.5}}>
                Renseigne ton lien de paiement personnel — indépendant de la plateforme, c'est ton compte à toi, ton argent va directement chez toi.
              </div>
              <div style={{fontSize:".64rem",fontWeight:700,color:"#0070BA",marginBottom:".3rem"}}>🅿️ PayPal.me</div>
              <div style={{display:"flex",alignItems:"center",border:"1px solid #D8CCFF",borderRadius:8,marginBottom:".6rem",overflow:"hidden"}}>
              ...
              <div style={{fontSize:".64rem",fontWeight:700,color:"#635BFF",marginBottom:".3rem"}}>💳 Lien de paiement Stripe (optionnel)</div>
              <input value={lienStripePerso} onChange={e=>setLienStripePerso(e.target.value)} placeholder="https://buy.stripe.com/..."
                style={{width:"100%",border:"1px solid #D8CCFF",borderRadius:8,padding:".42rem .65rem",fontSize:".76rem",fontFamily:"inherit",marginBottom:".7rem",outline:"none"}}/>
              <button onClick={sauverPaiementPerso} disabled={paiementSaving}
                style={{width:"100%",background:paiementSaved?"#2E7D32":"#3D1F0E",color:"white",border:"none",borderRadius:8,padding:".5rem",fontSize:".76rem",fontWeight:700,fontFamily:"inherit",cursor:"pointer"}}>
              ...
              {(profil.packsPersonnalises||[]).length>0&&(
                <div style={{marginBottom:".7rem"}}>
                  {(profil.packsPersonnalises||[]).map(pack=>(
                    <div key={pack.id} style={{background:C.creme,border:`1px solid ${C.pale}`,borderRadius:10,padding:".6rem .75rem",marginBottom:".4rem",display:"flex",justifyContent:"space-between",alignItems:"center",gap:".6rem"}}>
                      <div style={{display:"flex",alignItems:"center",gap:".6rem",flex:1,minWidth:0}}>
                        {pack.photos&&pack.photos.length>0&&(
                          <div style={{display:"flex",flexShrink:0}}>
                            {pack.photos.slice(0,3).map((img,i)=>(
                              <img key={i} src={img} alt="" style={{width:38,height:38,borderRadius:8,objectFit:"cover",marginLeft:i>0?-14:0,border:"2px solid white",boxShadow:"0 1px 3px rgba(0,0,0,.15)"}}/>
                            ))}
                          </div>
                        )}
                        <div style={{minWidth:0}}>
                          ...
                          <div style={{fontSize:".65rem",color:C.gris}}>
                            {pack.produitsRefs.length} produit{pack.produitsRefs.length>1?"s":""} · {pack.prixPromo?(<><span style={{textDecoration:"line-through"}}>{pack.prix.toFixed(2)}€</span> <span style={{color:"#C44B1A",fontWeight:700}}>🏷️ {pack.prixPromo.toFixed(2)}€</span></>):(`${pack.prix.toFixed(2)}€`)}
                          </div>
                        </div>
                      </div>
                ...
                <div style={{background:C.creme,border:`1px solid ${C.pale}`,borderRadius:10,padding:".75rem"}}>
                  ...
                  {packPhotos.length>0&&(
                    <div style={{marginBottom:".5rem"}}>
                      <div style={{fontSize:".62rem",color:C.gris,marginBottom:".3rem"}}>Photos du pack (automatique, selon les produits cochés)</div>
                      <div style={{display:"flex",gap:".4rem",flexWrap:"wrap"}}>
                        {packPhotos.map((img,i)=>(
                          <img key={i} src={img} alt="" style={{width:50,height:50,borderRadius:8,objectFit:"cover",border:`1px solid ${C.pale}`}}/>
                        ))}
                      </div>
                    </div>
                  )}
                  <input value={packPrix} onChange={e=>setPackPrix(e.target.value)} placeholder="Prix du pack en € (ex: 29.90)" type="number" step="0.01"
                    style={{width:"100%",border:`1px solid ${C.pale}`,borderRadius:8,padding:".42rem .65rem",fontSize:".78rem",fontFamily:"inherit",color:C.texte,background:C.blanc,outline:"none",marginBottom:".45rem"}}/>
                  <label style={{display:"flex",alignItems:"center",gap:".5rem",marginBottom:".45rem",cursor:"pointer"}}>
                    <input type="checkbox" checked={packEnPromo} onChange={e=>{setPackEnPromo(e.target.checked);if(!e.target.checked)setPackPrixPromo("");}}/>
                    <span style={{fontSize:".74rem",color:C.texte,fontWeight:600}}>🏷️ Mettre ce pack en promotion</span>
                  </label>
                  {packEnPromo&&(
                    <input value={packPrixPromo} onChange={e=>setPackPrixPromo(e.target.value)} placeholder="Prix promo en € (ex: 24.90)" type="number" step="0.01"
                      style={{width:"100%",border:`1.5px solid #C44B1A`,borderRadius:8,padding:".42rem .65rem",fontSize:".78rem",fontFamily:"inherit",color:C.texte,background:"#FFF3EC",outline:"none",marginBottom:".45rem"}}/>
                  )}
                  {!catalogueProduits?(
                    <div style={{fontSize:".72rem",color:C.gris,padding:".5rem"}}>Chargement du catalogue...</div>
                  ):(<>
                    ...
                    <div style={{background:C.creme,borderRadius:8,padding:".5rem .7rem",marginBottom:".5rem",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <span style={{fontSize:".7rem",color:C.gris}}>Total des produits sélectionnés</span>
                      <span style={{fontSize:".9rem",fontWeight:700,color:C.brun}}>
                        {packProduits.reduce((s,ref)=>{const prod=catalogueProduits.find(cp=>cp.ref===ref);return s+(prod?.prix||0);},0).toFixed(2)}€
                      </span>
                    </div>
                      ...
                      {catalogueProduits.filter(p=>!filtrePack||p.nom.toLowerCase().includes(filtrePack.toLowerCase())).slice(0,60).map(p=>{
                        ...
                        return(
                          <label key={p.ref} style={{display:"flex",alignItems:"center",gap:".6rem",padding:".45rem .6rem",borderBottom:`1px solid ${C.pale}`,cursor:"pointer",background:selectionne?C.rose+"12":"transparent"}}>
                            <input type="checkbox" checked={selectionne} onChange={()=>{
                              const nouveauxRefs=selectionne?packProduits.filter(r=>r!==p.ref):[...packProduits,p.ref];
                              setPackProduits(nouveauxRefs);
                              const toutesImages=nouveauxRefs.map(ref=>catalogueProduits.find(cp=>cp.ref===ref)).filter(prod=>prod&&prod.image).map(prod=>prod.image);
                              setPackPhotos(toutesImages);
                            }}/>
                  ...
                  <div style={{display:"flex",gap:".4rem"}}>
                    <button onClick={()=>{
                        ...
                        const nouveauPack={id:"pack"+Date.now(),nom:packNom.trim(),prix:parseFloat(packPrix)||0,produitsRefs:[...packProduits],photos:packPhotos,
                          ...(packEnPromo&&packPrixPromo?{prixPromo:parseFloat(packPrixPromo)||0}:{})};
                        setProfil(p=>({...p,packsPersonnalises:[...(p.packsPersonnalises||[]),nouveauPack]}));
                        setPackNom("");setPackPrix("");setPackProduits([]);setFiltrePack("");setPackEnPromo(false);setPackPrixPromo("");setPackPhotos([]);setCreationPack(false);
                      }}
                    ...
                    <button onClick={()=>{setCreationPack(false);setPackNom("");setPackPrix("");setPackProduits([]);setFiltrePack("");setPackEnPromo(false);setPackPrixPromo("");setPackPhotos([]);}}
