...
exports.notifChallenge = onDocumentCreated("challenges/liste/items/{challengeId}", async (event) => {
  try {
    ...
    if (c.global || !c.equipesCibles || c.equipesCibles.length === 0 || c.equipesCibles.includes("all")) {
      ...
      const annSnap = await db.collection("equipe").doc("annuaire").get();
      const ann = annSnap.exists ? (annSnap.data().membres || {}) : {};
      ...
      c.equipesCibles.forEach(chefUid => {
        ...
        getDescendants(chefUid).forEach(u => recipients.add(u));
      });
