...
fs.writeFileSync(f + '.bak-final', c);
let ok = 0;
...
if (c.includes('useState(initialSection)') || c.includes('useState(initialSection||')) { ok++; console.log('B DEJA'); }
...
// E - onglet Boutique dans la nav interne
const p5 = '{id:"liens",icon:"\uD83D\uDD17",label:"Liens"}';
if (c.includes('"id":"boutique"') || (c.includes('id:"boutique"') && c.includes('label:"Boutique"'))) { ok++; console.log('E DEJA'); }
else if (c.includes(p5)) {
  c = c.replace(p5, p5 + ',\n    {id:"boutique",icon:"\uD83D\uDECD\uFE0F",label:"Boutique"}');
  ok++; console.log('E OK');
} else console.log('E ECHEC');

// F - section boutique apres ebooks
const p6 = 'activeSection==="ebooks"&&(<EbooksLinkBioSection';
...
if (c.includes('TOKENS-SECTION') || c.includes('activeSection==="boutique"')) { ok++; console.log('F DEJA'); }
else if (c.includes(p6)) {
  c = c.replace(p6, section + p6);
  ok++; console.log('F OK');
